...
static esp_err_t http_server_set_color_handler(httpd_req_t *req) {
    ESP_LOGI(TAG, "/set_color requested");

    // Buffer para recibir los datos POST
    char buf[100];
    ...
    buf[ret] = '\0';  // Terminar la cadena correctamente

    // Parsear los datos JSON recibidos, por ejemplo: {"red":255,"green":100,"blue":50}
    sscanf(buf, "{\"red\":%d,\"green\":%d,\"blue\":%d}", &red, &green, &blue);

    // Establecer el color del LED RGB con los valores recibidos
    rgb_led_set_color(red, green, blue);

    // Responder al cliente que la operación fue exitosa
    httpd_resp_set_type(req, "application/json");
    httpd_resp_send(req, "{\"status\":\"Color set\"}", HTTPD_RESP_USE_STRLEN);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
		...
		if (strncmp(event->data, "toggle", event->data_len) == 0){
		 ...
		 // Si el tema es para cambiar el color del LED
        if (strncmp(event->topic, "led/color", event->topic_len) == 0) {
            int red, green, blue;
            // Parsear los valores RGB del mensaje
            sscanf(event->data, "{\"red\":%d,\"green\":%d,\"blue\":%d}", &red, &green, &blue);
            rgb_led_set_color(red, green, blue); // Aplicar el color al LED
        }
...
static httpd_handle_t http_server_configure(void)
{
	...
	httpd_register_uri_handler(http_server_handle, &dht_sensor_json);
		...
		// Register set_color handler for RGB LED control
